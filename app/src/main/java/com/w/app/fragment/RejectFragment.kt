...
class RejectFragment:BaseFragmnet() {
    ...
    override fun Init(view: View) {
        ...
        rvRejected!!.addOnScrollListener(object : RecyclerView.OnScrollListener() {
                ...
                if (dy > 0) { //check for scroll down
                    ...
                    if (isLoding) {
                        if (visibleItemCount + pastVisibleItem >= totalItemCount) {
                            isLoding = false
                            currentPage ++
                            callApiRejectedWallPaper(false)
                        }
                    }
                }
            }
    ...
    private fun callApiRejectedWallPaper(isFristPage: Boolean) {
    ...
    fun setPendingAdaptor(
        ...
        if (isFristPage) {
            ...
            rvRejected!!.adapter = ImageAdaptor
            rvRejected!!.layoutManager = manager
            rvRejected!!.itemAnimator = DefaultItemAnimator()
            rvRejected!!.isNestedScrollingEnabled = false
        } else {
