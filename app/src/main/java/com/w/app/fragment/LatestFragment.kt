...
class LatestFragment : BaseFragmnet() {
    ...
    override fun setView(): Int {
        return R.layout.fragment_leatest
    }
    ...
    override fun Init(view: View) {
        ...
        if (Common.isCheckNetwork(activity!!)) {
            callApiLatestWallPaper(true, false)
        } else {
        ...
        rvLatest!!.addOnScrollListener(object : RecyclerView.OnScrollListener() {
                ...
                if (dy > 0) { //check for scroll down
                    ...
                    if (isLoding) {
                        if (visibleItemCount + pastVisibleItem >= totalItemCount) {
                            isLoding = false
                            currentPage++
                            callApiLatestWallPaper(false, false)
                        }
                    }
                }
            }
    ...
    private fun callApiLatestWallPaper(isFristPage: Boolean, isFavourite: Boolean) {
        ...
        val call = ApiClient.getClient.getLatestwallpapers(map)
    ...
    fun setLatestAdaptor(
        ...
        if (isFristPage) {
            ImageAdaptor =
                object : BaseAdaptor<WallpaperModel>(activity!!, imageList) {
                    override fun onBindData(
                     ...
                     //  Common.getLog("getColor",Integer.toHexString(Integer.parseInt(wallpaperList.get(position).getWallpaper_color().toString()))+"----")
                        ...
                        if (wallpaperList!!.get(position).getIsFavourite().equals("1")) {
                            holder.itemView.iv_btn_fevourite.setImageDrawable(
                                resources.getDrawable(
                                    R.drawable.ic_fillheart
                                )
                            )
                            holder.itemView.iv_btn_fevourite.imageTintList =
            ...
            rvLatest!!.adapter = ImageAdaptor
            rvLatest!!.layoutManager = manager
            rvLatest!!.itemAnimator = DefaultItemAnimator()
            rvLatest!!.isNestedScrollingEnabled = false
        } else {
    ...
    override fun onResume() {
        ...
        if (SharePreference.getBooleanPref(activity!!, SharePreference.isFavourite)) {
            ...
            isLoding = true
            if (Common.isCheckNetwork(activity!!)) {
                callApiLatestWallPaper(true, true)
            } else {
                Common.alertErrorOrValidationDialog(
                    activity!!,
                    resources.getString(R.string.no_internet)
